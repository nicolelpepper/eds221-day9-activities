...
# ANSWER: HAD TO LOAD HERE PACKAGE
...
```{r, include = FALSE}
#CORRECTED SPECIES TO SPECIES_ID
birds_df <- birds_1 %>% 
...
bird_totals_by_park <- birds_df %>% 
  left_join(sites_1) %>% 
  group_by(park_name) %>% # CORRECTED GROUPBY TO GROUP_BY
...
ggplot(data = bird_totals_by_park, aes(x = total_count, y = park_name)) +
  ...
  labs(x = "Total bird count",
       y = "Park name") +  # CHANGED PIPE TO PLUS +
  theme_minimal()
```
...
common_birds <- birds_df %>% 
  full_join(sites_1) %>%  # changed semi join to full
...
selected_parks <- common_birds %>% 
  filter(park_name %in% c("Cholla Cove", "Desert Willow", "Nueve", "Orme", "Palomino", "Sonrisa")) # corrected Palomino spelling
```

The code below creates a faceted ggplot graph with the top 5 most frequently observed birds at each location. 

```{r}
ggplot(data = selected_parks, aes(y = common_name, x = total_count)) + # added aes()
  geom_col() +
...
```{r}
ggsave("bird_counts.jpg", height = 6, width = 12) # correct height
```
